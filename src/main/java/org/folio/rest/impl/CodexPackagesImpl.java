...
import org.folio.codex.LHeaders;
...
public class CodexPackagesImpl implements CodexPackages {
  ...
  @Override
  public void getCodexPackagesById(String id, String lang, Map<String, String> okapiHeaders, Handler<AsyncResult<Response>> handler, Context vertxContext) {
    logger.info("CodexPackagesImpl#getCodexPackagesById");
    LHeaders headers = new LHeaders(okapiHeaders);
    okapiClient.getModuleList(vertxContext, headers, CodexInterfaces.CODEX_PACKAGES)
      .compose(modules ->
        okapiClient.getOptionalObjects(vertxContext, headers, modules,
          headers.get(XOkapiHeaders.URL) + "/codex-packages/" + id, Package.class))
      .map(instances -> {
